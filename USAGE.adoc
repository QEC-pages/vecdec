...
weight-two errors are stored in the hash.

- **Use Belief Propagation (BP) disorder with optional OSD.** Same
  code, using up to `steps=10` BP iterations with parallel update
...
decoder.  This can be improved a bit by increasing the OSD level.
Interestingly, larger number of steps can actually decrease the
performance of parallel BP decoder: 
