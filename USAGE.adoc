...
[source,sh]
include::examples/ex_C1_sh.out[lines="11..-1"]
...
This will create a DEM file `tryD.dem` with the probability values
scaled by a factor `mulP=2`.  The file can be used, e.g., as an input to
`Stim`, to generate errors/det/obs files externally.
...
[source,sh]
===
paste -d: tmp_det.01 tmp_obs.01 > tmp1.01
paste -d: tmp_D.01 tmp_L.01 > tmp2.01
paste tmp1.01 tmp2.01 -d" "| grep -E -c -v '([01]+:[01]+) \1'
===
