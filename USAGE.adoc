...
`P_FAIL=0.033` (decoding failed `FAIL=33` times out of `NTOT=1000`
...
`maxosd=50` columns for OSD decoding:
  
[source,sh]
include::examples/ex_B1.sh[]

[source,sh]
include::examples/ex_B1_sh.out[]
...
==== Simulate the performance of a quantum code
...
setting `seed=0`) after reading the codewords.

[source,sh]
include::examples/ex_D2.sh[]

[source,sh]
include::examples/ex_D2_sh.out[]

In a particular run, `210918` codewords were written to the file.  The computed codewords
