...
echo "#" d p 'pF(vecdec) ' 
for (( dd=$dmin; dd<=$dmax; dd+=2 )) ; do # distance loop
    fnam=surf_d$dd # filename to use
    ...
    for (( j1=$jmin; j1<=$jmax; j1+=2 )) ; do #   p1 loop (depolarizing)
      ...
      #      pyF=`paste -d " " p$obsf $obsf | grep "0 1\|1 0" | wc -l`
      echo "# pymatching :" $dd $p1 `awk "BEGIN { print \"  \" $pyF \" \" $pyF/$Ntotal \"    \" $pyFv \" \" $pyFv/$Ntotal \" \" $Ntotal }"`
      # use the same sample in `vecdec`
      if ((dd==3)) ; then steps_max=16000 ;
      elif ((dd==5)) ; then steps_max=64000 ; # this is going to be very slow, but I want to know how bad `pymatching` actually is
      elif ((dd==7)); then steps_max=256000 ; fi
      ...
      echo $dd $p1 `awk '{print $1/$2 " " $1 }' < $fnam.out` \
