...
csr_t * do_K_from_C(const csr_t * const mLt, const one_vec_t * const codewords,
    ...
    if (k <= 0) {
        ERROR("The number of rows to construct (k) must be positive.");
        return NULL;
    }else
    printf("The numer of rows is%d\n",k);
    if (n <= 0) {
        ERROR("The number of columns (n) must be positive.");
        return NULL;
    }
    ...
    if (rank == 0) {
        printf("Warning: The resulting matrix has no rows (rank is 0).\n");
    }
